"""
Weinvisualisierung - Bildgenerierung mit PIL/NumPy
Kopiert aus colours_of_wine für eigenständige Nutzung.
"""
...
def draw_residual_sugar_bar(img: Image.Image, residual_sugar: float, bar_width: int = None) -> Image.Image:
    ...
    if bar_width is None:
        bar_width = max(int(w * 0.05), 30)
    
    new_w = w + bar_width
    new_img = Image.new("RGB", (new_w, h), (252, 252, 254))
...
def generate_wine_png_bytes(viz: dict, size: int = 512) -> bytes:
    """Generiert ein PNG als Bytes."""
    ...
    if is_red_wine:
        ...
        center_weight = np.clip(1 - t, 0, 1) ** 1.8
